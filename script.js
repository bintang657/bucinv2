...
// 2. KURSOR & CLICK SPARKS 
const cursorDot = document.getElementById("cursor-dot");
const cursorOutline = document.getElementById("cursor-outline");
const magneticBtns = document.querySelectorAll('.magnetic-btn');

// Deteksi apakah ini HP atau Laptop
...
window.addEventListener("mousemove", function (e) {
    ...
    // Efek Magnetik (Hanya efektif di Laptop/PC)
    if(!isMobile) {
...
window.addEventListener('click', (e) => {
    // Kurangi jumlah spark di HP agar mulus
    ...
    for(let i=0; i<sparkCount; i++) {
        ...
        let angle = (Math.PI * 2 / sparkCount) * i; let distance = Math.random() * 40 + 20;
...
createParticles('hearts-bg', 'heart', isMobile ? 5 : 8, 12);
createParticles('petals-bg', 'petal', isMobile ? 8 : 15, 15);
...
window.tekanTombolKangen = function() {
    confetti({ particleCount: isMobile ? 100 : 200, spread: 180, origin: { y: 0.5 }, colors: ['#800000', '#38bdf8', '#ffffff', '#ff0000', '#ffdf00'] });
...
window.bukaFoto = function(url, pesan) {
    Swal.fire({
        ...
        title: '<span class="text-maroon-light font-serif text-3xl drop-shadow-md">Cantik Banget</span>',
        html: `<p class="text-gray-200 italic text-lg mt-4 leading-relaxed">${pesan}</p>`,
        ...
        customClass: { image: 'rounded-xl shadow-[0_0_30px_#38bdf8] border-2 border-blue-accent', popup: 'border-2 border-maroon shadow-[0_0_30px_#800000]' },
...
// 6. 3D BACKGROUND (DIOPTIMALKAN UNTUK HP ANTI-LAG)
const init3D = () => {
    ...
    // Paksa Pixel Ratio 1 untuk HP agar ringan
    ...
    const cMaroon = new THREE.Color('#a52a2a'); 
    ...
    const matOpts = { size: 0.08, vertexColors: true, transparent: true, opacity: 0.8 };
    if(!isMobile) matOpts.blending = THREE.AdditiveBlending;
    ...
    const animate = () => {
        ...
        // Rotasi konstan yang sangat mulus
